...
- **Sourced** badge for descriptions backed by external metadata snippets; tap to see the source URLs.
...
- **Grounded book descriptions** — `BookDescriptionService` queries a `CompositeMetadataLookupService` that chains Google Books (primary, requires API key) and Open Library (fallback) through `IBookMetadataLookupService`, feeds matched snippets into the prompt, and instructs the model to use only those sources. Books with no matching sources get `Description unavailable`. Each saved description records its sources, generation time, and a grounded flag, surfaced as the Library "Sourced" badge.
...
Services/
  ...
  BookDescriptionService.cs       # Grounded short/long descriptions via metadata snippets
  BookMetadataLookupService.cs    # IBookMetadataLookupService + Open Library provider (fallback)
  GoogleBooksMetadataLookupService.cs  # Google Books provider (primary, requires API key)
  CompositeMetadataLookupService.cs    # Chains providers; stops once a description is found
