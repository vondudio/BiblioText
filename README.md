...
- **On-device AI** — YOLO detection, Whisper.net dictation, Windows AI `TextRecognizer` OCR, and content-index search all work without a network.
...
- **Library** — Browse all saved books with spine thumbnails, detection index, on-device semantic search, duplicate badge, per-provider source badges (G / W / OL / AI), status filter (All / Duplicates / Missing Descriptions), location filter, sort options, inline editing, and a light-dismiss description popup showing the long description plus source URLs.
